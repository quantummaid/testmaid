...
class TraceId(unsafe: String) : ValidatedString(validators, unsafe) {
    companion object {
        private val validators = allOf(length(1, 128), regex("[\\w-_](/?[\\w-_])*".toRegex()))
    }

    fun extend(subTraceId: TraceId): TraceId {
        return TraceId("${mappingValue()}/${subTraceId.mappingValue()}")
    }
}