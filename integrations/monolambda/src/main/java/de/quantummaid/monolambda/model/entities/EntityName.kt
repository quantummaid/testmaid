...
class EntityName(unsafe: String) : ValidatedString(validators, unsafe) {
    fun upperCased(): String {
        return safeValue.camelToSnakeCase()
    }

    fun tableName(): DynamoDbTableName {
        return DynamoDbTableName(safeValue)
    }

    fun envVariableName(): EnvVariableName {
        return EnvVariableName(safeValue.camelToSnakeCase())
    }
