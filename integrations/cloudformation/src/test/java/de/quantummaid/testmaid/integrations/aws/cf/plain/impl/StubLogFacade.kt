...
class StubLogFacade(val logToStdOut: Boolean) : CloudFormationServiceLogFacade, RCALogFacade {
    ...
    override fun infoCreatedStack(createdStack: CreatedStack, timeout: Duration) {
        logToStdOut("Created Stack ${createdStack.stackName}")
    }
    ...
    override fun infoUpdatedStack(createdStack: CreatedStack, timeout: Duration) {
        logToStdOut("Updated Stack ${createdStack.stackName}")
    }
